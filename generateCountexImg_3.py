...
def train_fn(disc, gen, vqa_model, loader, dataset, opt_disc, opt_gen, l2, cross_entropy, bce, g_scaler, d_scaler, epoch, L2_LAMBDA, CE_LAMBDA, tb):
    ...
    for idx, (img, qid) in enumerate(loop):
        ...
        #error = (torch.abs(pred_logits_new - orig_logits_new)).sum()
        #squared_error = ((pred_logits_new - orig_logits_new) * (pred_logits_new - orig_logits_new)).sum()
        
        running_mae = error
        running_mse = squared_error
        
        rmse = math.sqrt(running_mse/len(loader))
        numOfLoader = len(loader)
        mae = running_mae/len(loader)
        #std = torch.std(pred_logits_new - orig_log)
        
        #訓練，検証データ数テスト
        print(len(loader))

        shapeof_orig_logits = orig_logits.shape
        shapeof_orig_logits_new = orig_logits_new.shape
        shapeof_pred_logits = pred_logits.shape
        shapeof_pred_logits_new = pred_logits_new.shape
        shapeof_answer = answer.shape
        import pdb; pdb.set_trace()
...
class VQADataset2(Dataset):
    ...
    def __getitem__(self, index):
        ...
        #mask = npmask(annotation_item, 256, 256, np.nan)
        #logits = self.model(img, question_item)
        #q_emb = self.activation['seq2vec']
        
        return img, q_id
        #return img, mask, logits, q_emb, q_id
    ...
    for CE_LAMBDA in config.CE_LAMBDA:
        for L2_LAMBDA in config.L2_LAMBDA:
            ...
            for epoch in range(config.NUM_EPOCHS):
                #G_losses, D_losses, D_fake_losses, D_real_losses = 
                train_fn(disc, gen, vqa_model, train_loader, train_dataset, opt_disc, opt_gen, L2_LOSS, CROSS_ENTROPY, BCE, g_scaler, d_scaler, epoch, L2_LAMBDA, CE_LAMBDA, tb)
                ...
                #    os.mkdir("Gradients")
                #grad = gradient_penalty(nn.L1Loss, real, fake, logit_new, logit_old, config.DEVICE)                 

...
def train_fn(disc, gen, vqa_model, loader, dataset, opt_disc, opt_gen, l2, cross_entropy, bce, g_scaler, d_scaler, epoch, L2_LAMBDA, CE_LAMBDA, tb):
    ...
    for idx, (img, qid) in enumerate(loop):
        ...
        with torch.cuda.amp.autocast():                    
            ...
            #image = torch(generated).unsqueeze(0)
            #plt.imshow(image[0])
            #test1 = torchvision.transforms.functional.to_pil_image(generated)
            #test1.save()
            #test = transforms.ToPILImage()(image)
            #test.show()

            #tensor = torch.zeros((64, 128, 3))
            #test = tensor.to('cpu').detach().numpy()
            #test.save()
                
  #          output = model(generated)
  #          output = outpu.cpu().numpy()
  #          cv2.imwrite(output, 'test1.png')
        ...
        img = img.to(config.DEVICE)
        try:
            #'./tools/data/vqa/OpenEnded_mscoco_train2014_questions.json'
            save_image(gen_im, f"test.png")
            save_image(img, f"test_orig.png")

            #save_image(image_to_save, f"L2-lambda_{L2_LAMBDA}_CE-lambda_{CE_LAMBDA}/y_gen_q_{str(question) + ' '+ans_new}_{epoch}.png")
            #save_image(img, f"L2-lambda_{L2_LAMBDA}_CE-lambda_{CE_LAMBDA}/orig_q_{str(question)+' '+ans}_{epoch}.png")
        except:
            pass
        #del image_to_save
        ...
        if epoch > 0 and idx % 1 == 0:
            ...
            D_real = torch.tensor(0, dtype=torch.float64)
            D_fake = torch.tensor(0, dtype=torch.float64)
            D_loss = torch.tensor(0, dtype=torch.float64)
